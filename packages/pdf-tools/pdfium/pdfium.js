var PDFiumModule = (() => {
	...
	return function (moduleArg = {}) {
		...
		if (ENVIRONMENT_IS_NODE) {
			...
			readBinary = filename => {
				...
				const result = nodePath.normalize(`${process.cwd()}/.next/${filename.href.replace(isVercel ? '' : '/_next', '')}`)
				filename = !ENVIRONMENT_IS_NODE ? new URL(filename) : result;
    ...
    function findWasmBinary() {
      return new URL('pdfium.wasm', import.meta.url);
    }
		var wasmBinaryFile;
		...
		function instantiateAsync(binary, binaryFile, imports, callback) {
			if (
				...
				(typeof URL === 'function' && binaryFile instanceof URL) &&
				!ENVIRONMENT_IS_NODE &&
